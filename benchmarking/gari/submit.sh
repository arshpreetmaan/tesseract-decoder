...
for num in $(seq 0 20); do
  for p_err in 0.001; do
    ...
    # for circuit in testdata/surfacecodes/r=7,*p=$p_err,noise=si1000,c=surface_code_Z,*cz.stim; do
    #   echo "$circuit"
    # done
  done
  ...
  for L_type in "" "ogL_"; do
    for mode in modeA modeG modeI modeK modeN modeO; do
      dem_file="$circuit_dir/gari/${circuit_name}_${L_type}${mode}.dem"
      echo "Running GARI mode: $dem_file"
      for order in order2 order4 order7 order9 order10 all; do
